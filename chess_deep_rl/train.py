...
class Trainer:
    ...
    def plot_loss(self, history):
        ...
        plt.savefig(f"{config.LOSS_PLOTS_FOLDER}/loss1.png")
        print("loss plotted")

    def save_model(self):
        os.makedirs(config.MODEL_FOLDER, exist_ok=True)
        # path = f"{config.MODEL_FOLDER}/model-{datetime.now().strftime('%Y-%m-%d_%H:%M:%S')}.h5"
        path = f"{config.MODEL_FOLDER}/model2.h5"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model',default=os.path.join(config.MODEL_FOLDER, "model.h5"), type=str, help='The model to train')
    parser.add_argument('--data-folder',default=os.path.join(config.MEMORY_DIR),type=str, help='The data folder to train on')
